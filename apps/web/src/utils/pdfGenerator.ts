...
export const generateInvoicePDF = (sale: Sale) => {
  ...
  pdf.text(`Canal de venta: ${translateSalesChannel(sale.salesChannel)}`, 20, 128);
  pdf.text(`Método de pago: ${translatePaymentMethod(sale.paymentMethod)}`, 20, 136);
  pdf.text(`Estado del pago: ${translatePaymentStatus(sale.paymentStatus)}`, 20, 144);
  ...
  pdf.text(`Estado: ${translateSaleStatus(sale.status)}`, 120, 210);
...
/**
 * Get color for sale status (used in PDF generation)
 * @private
 */
